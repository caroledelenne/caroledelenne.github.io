...
def parse_supervision(supervision_dir):
    """Parse supervision items from the _supervision directory."""
    supervision = []
    
    if not os.path.exists(supervision_dir):
        return supervision
    
    for supervision_file in sorted(glob.glob(os.path.join(supervision_dir, "*.md"))):
        with open(supervision_file, 'r', encoding='utf-8') as file:
            content = file.read()
        ...
        if front_matter_match:
            ...
            # Extract supervision details
            supervision_entry = {
                "name": front_matter.get('title', ''),
                "category": front_matter.get('collection', 'supervision'),
                "date": front_matter.get('date', ''),
